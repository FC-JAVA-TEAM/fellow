...
public class DailyNotesView extends Div implements RouterLayout {

    private final DailyNoteServiceXX dailyNoteService;
    private TreeGrid<DailyNoteXX> treeGrid; // Make treeGrid a class member
    private Set<DailyNoteXX> expandedItems = new HashSet<>(); // Set to keep track of expanded items

    public DailyNotesView(DailyNoteServiceXX dailyNoteService) {
    ...
    private void init() {
        ...
        Label titleLabel = new Label("Daily Notes");
        ...
        treeGrid.addColumn(DailyNoteXX::getContent).setHeader("Daily Note");
        ...
        treeGrid.addItemClickListener(event -> {
            DailyNoteXX selectedNote = event.getItem();
    ...
    private void updateDailyNotesGrid() {
        ...
        List<DailyNoteXX> dailyNotes = dailyNoteService.findByMonth(currentDate);

        // Store currently expanded items before the update
        Set<DailyNoteXX> currentExpandedItems = new HashSet<>(expandedItems);
        expandedItems.clear(); // Clear the previous expanded items

        // Structure the data for TreeGrid
        TreeData<DailyNoteXX> treeData = new TreeData<>();
        Map<LocalDate, List<DailyNoteXX>> notesByDate = dailyNotes.stream()
                .collect(Collectors.groupingBy(DailyNoteXX::getDate));

        for (LocalDate localDate : notesByDate.keySet()) {
            List<DailyNoteXX> notes = notesByDate.get(localDate);
            DailyNoteXX parentNote = new DailyNoteXX(); // Create a dummy parent object for the date
    ...
    private void openEditDailyNoteDialog(DailyNoteXX note, Runnable onUpdate) {
        DailyNoteForm dailyNoteForm = new DailyNoteForm(dailyNoteService);
        dailyNoteForm.openEditDailyActivityDialog1(note, onUpdate); // Pass the update callback
    }
}
